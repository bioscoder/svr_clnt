...
int main()
{
	...
	socklen_t socklen;
	int listener;
	char incoming_data[MAX_BUFFER_SIZE];
	int sock;
	...
	while (!srv_incomingSignal)
	{
		...
		for(int current_socket = 0; current_socket <= max_seen_socket; current_socket++)
		{
			if (FD_ISSET(current_socket, &read_set_fd))
			{
				...
				{
					if ((retv_recv = recv(current_socket, incoming_data, MAX_BUFFER_SIZE, 0)) <= 0)
					{
						if (retv_recv == 0)
					...
					else
					{
						...
						if (!cOnline[i].inUse)
						{
							if (getInfoFromHeader(incoming_data,retv_recv, &cOnline[i]))
							{
								cOnline[i].inUse = 1;
								printf("client %d[%d]: data size: %d, compression:%d level:%d\n",
									i, cOnline[i].c_socket, cOnline[i].c_targetSize, cOnline[i].c_compressionType,cOnline[i].c_compressionLevel);
							}
						}
						processAndSend(current_socket, incoming_data, retv_recv, &cOnline[i]);

						if ((cOnline[i].c_rxSize == cOnline[i].c_targetSize) || (cOnline[i].c_error))
						{
							showCstat(&cOnline[i]);
